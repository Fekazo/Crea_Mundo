                ...
                <div className="card-kids border-0 p-8 animate-pop-in">
                    ...
                    <div className="mb-7 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-semibold text-slate-500">
                                Crea una nueva materia educativa
                            </p>
                        </div>
                    ...
                    <form onSubmit={submit} className="space-y-5">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-sm font-bold text-slate-700">Nombre de la materia *</label>
                            <input
                                type="text"
                                value={data.name}
                        ...
                        <div className="space-y-1.5">
                            <label className="text-sm font-bold text-slate-700">Color de la materia</label>
                        ...
                        {data.name && (
                            <div
                                className="flex items-center gap-3 rounded-2xl p-4 animate-slide-up"
                                style={{ background: `${data.color}15`, border: `2px solid ${data.color}30` }}
                            >
                                <div
                                    className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl shadow-sm"
                                    style={{ background: `linear-gradient(135deg, ${data.color} 0%, ${data.color}BB 100%)` }}
                                >
                                    <BookOpen className="h-5 w-5 text-white" strokeWidth={2} />
                                </div>
                                <div>
                                    <p className="font-bold text-slate-800" style={{ fontFamily: 'Fredoka, sans-serif' }}>
                                        {data.name}
                                    </p>
                                    {data.description && (
                                        <p className="text-xs font-medium text-slate-500 line-clamp-1">{data.description}</p>
                                    )}
                                </div>
                            </div>
                        )}
...
SubjectsCreate.layout = {
    breadcrumbs: [
        { title: 'Materias', href: subjects.index() },
        { title: 'Nueva', href: subjects.create() },
    ],
};
